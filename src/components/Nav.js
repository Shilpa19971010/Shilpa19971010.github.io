    ...
    useEffect(() => {
        window.addEventListener('scroll', () => {
            ...
            return () => {
                ...
                console.log('guess it got unmounted')
            }

        })
    }, [])

    // let handleShow = (toShow) => {
    //     let nav = document.getElementByClassName('nav');
    //     toShow ? nav.classList.add('topbar') : nav.classList.remove('topbar');
    // }

    return (
        <div className={`nav ${show && "nav_topbar"}`}>
